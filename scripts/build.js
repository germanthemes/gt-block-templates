...
const fileBuildJS = path.resolve( theCWD, './assets/js/gt-block-templates-editor.js' );
const fileEditorCSS = path.resolve( theCWD, './assets/css/gt-block-templates-editor.css' );
const fileStyleCSS = path.resolve( theCWD, './assets/css/gt-block-templates.css' );
...
async function build( webpackConfig ) {
	...
	compiler.run( ( err, stats ) => {
		...
		console.log(
			...
			`${ chalk.dim( '— ./assets/js/' ) }`,
			`${ chalk.green( 'gt-block-templates-editor.js' ) }`,
			'\n',
			getFileSize( fileEditorCSS ),
			`${ chalk.dim( '— ./assets/css/' ) }`,
			`${ chalk.green( 'gt-block-templates-editor.css' ) }`,

			'\n',
			getFileSize( fileStyleCSS ),
			`${ chalk.dim( '— ./assets/css/' ) }`,
			`${ chalk.green( 'gt-block-templates.css' ) }`,
